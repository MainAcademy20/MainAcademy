...
from models import Candidates_list, db


scheduler = BackgroundScheduler()

db.create_tables([Candidates_list])
...
class Ui_MainWindow(object):
    ...
    def candidates(self):
        candidate_ln = self.lineEdit_2.text()
        candidate_fn = self.lineEdit.text()
        if not candidate_ln and not candidate_fn:
            return
        status = self.comboBox
        human,_ = Candidates_list.get_or_create(last_name=candidate_ln, first_name=candidate_fn)
        if human.status!= status:
            human.status = status
            human.save()
